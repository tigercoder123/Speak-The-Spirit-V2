...
import { RUSHING_WATERS_CONFIG } from '../../../utils/questPrompts';
...
export default function RushingWatersScene({ onComplete }: { onComplete?: () => void }) {
  ...
  const {
    ...
    handleAskGloo,
    beginChallenge,
  } = useQuestChallenge(RUSHING_WATERS_CONFIG);
      ...
      <div className="w-[60%] h-full bg-slate-900 relative flex flex-col items-center border-r-4 border-black text-white p-4">
        ...
        {(stageState !== 'lock-challenge') ? (
          <RushingWatersStage
            ...
            onOpenChest={async () => {
              await beginChallenge("To unlock the chest, let's learn what 'things not seen' actually means!");
            }}
          />
        ) : (
